...
const RecentOrders = ({ orders = [], totalCount }) => {
  ...
  const { displayOrders, totalAmount, hasOrders } = useMemo(() => {
    ...
    // If totalCount is provided, we use the display orders' amount for summary,
    // but the footer usually shows total orders count.
    const currentTotalAmount = orders.reduce((sum, order) => {
      const amount =
    ...
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 flex flex-col h-full">
      ...
      {hasOrders && (
        <div className="mt-auto pt-6 border-t border-gray-200">
          <div className="flex items-center justify-between">
            <div className="text-sm text-gray-500">
              Showing {displayOrders.length} of {totalCount || orders.length}{" "}
              orders
            </div>
            <div className="text-right">
